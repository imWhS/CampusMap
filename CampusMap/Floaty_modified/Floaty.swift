...
open class Floaty: UIView {
  ...
  @objc open var size: CGFloat = 56 {
    didSet {
      ...
      self.recalculateItemsOrigin() //Calculate items position using button size
    }
  }
   ...
   */
  @objc @IBInspectable
  open var rotationDegrees: CGFloat = 0
   ...
   */
  @objc @IBInspectable
  open var buttonColor: UIColor = UIColor(red: 73/255.0, green: 151/255.0, blue: 241/255.0, alpha: 0)
   ...
   */
  @objc @IBInspectable
  open var plusColor: UIColor = UIColor(white: 0.2, alpha: 0)
  
  /**
     Background overlaying color.UIView *subV =  [[subView alloc] init];
     subV.backgroundColor = [UIColor redColor];
     subV.frame = CGRectMake(0, 0,[UIScreen mainScreen].bounds.size.width, 44);
     self.navigationItem.titleView = subV;
   */
  @objc @IBInspectable
   ...
   */
  @objc @IBInspectable
  open var itemSize: CGFloat = 42 { //modified (before 45)
    didSet {
  ...
  */
  ...
  fileprivate func determineTapArea(item : FloatyItem) -> CGRect {
    let tappableMargin : CGFloat = 30.0 //modified! (before 15.0)
    var x : CGFloat?
    if(item.titleLabelPosition == .left) {
      x = item.titleLabel.frame.origin.x + item.bounds.origin.x
    } else {
      x = item.bounds.origin.x //modified! (before x - 10)
    }
    let y = item.bounds.origin.y //modified! (before x - 5)
    ...
    let height = item.bounds.size.height //modified! (before height - 3)
  ...
  fileprivate func setButtonImage() {
    ...
    buttonImageView.layer.shadowColor = UIColor.black.cgColor
    buttonImageView.layer.shadowOffset = CGSize(width: 0, height: 3.5)
    buttonImageView.layer.shadowRadius = 4
    buttonImageView.layer.shadowOpacity = 0.28
    buttonImageView.frame = CGRect(
      ...
      height: buttonImageView.frame.size.height
        //x: circleLayer.frame.origin.x - 10.5,
        //y: circleLayer.frame.origin.y - 10.5
    )
  ...
  fileprivate func setTintLayer() {
    ...
    //before     tintLayer.frame = CGRect(x: circleLayer.frame.origin.x - 10.5, y: circleLayer.frame.origin.y - 10.5, width: 45, height: 45)
    tintLayer.backgroundColor = UIColor.black.withAlphaComponent(0.4).cgColor
  ...
  fileprivate func setRightBottomFrame(_ keyboardSize: CGFloat = 0) {
    ...
    var horizontalMargin = size; //before size - 3.5
    NSLog("Floaty horizontalMargin : \(horizontalMargin)")
    //var verticalMargin = size + keyboardSize;
    var verticalMargin = size;
    NSLog("Floaty verticalMargin : \(verticalMargin)")
  ...
  fileprivate func recalculateItemsOrigin() {
    for item in items { //size = 45
      let big = size > item.size ? size : item.size //big is button, small is item
      let small = size <= item.size ? size : item.size
      item.frame.origin = CGPoint(x: big/2 - small/2, y: big/2 - small/2)
    }
  }
...
extension Floaty {
  ...
  fileprivate func popAnimationWithOpen(group: DispatchGroup) {
    ...
    for item in items {
      ...
      group.enter()
      UIView.animate(withDuration: 3.3, delay: delay,
                     usingSpringWithDamping: 0.55,
                     initialSpringVelocity: 0.3,
                     options: UIView.AnimationOptions(), animations: { () -> Void in
                      item.layer.transform = CATransform3DIdentity
  ...
  fileprivate func slideUpAnimationWithOpen(group: DispatchGroup) {
    ...
    for item in items {
      ...
      UIView.animate(withDuration: 0.5, delay: 0, options: [], animations: { () -> Void in
        item.frame.origin.x = -itemHeight //before -itemHeight + 10
        item.alpha = 1
      }, completion: { _ in
        group.leave()
      })
